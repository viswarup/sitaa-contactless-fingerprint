...
[Downloand the application from the following link:] (https://drive.google.com/file/d/1rRBUuB_aEeAbNZIXqLuIC7Uf640Ujrdm/view?usp=sharing)
...
- **Track B (Implemented): Enhancement**
    ...
    - *Experiment 1* : We experimented with **Gabor Filter, Skeletonization(Thinning Ridges), and Minutiae Detection** but it did not give good results,it is included in the application just for demonstration. A report regarding our experimenation with tradional Computer Vision Methods is in the repo with the file name "traditional_CV_experimentation_report.pdf".
    - *Experiment 2*: **MobileNetV2 with U-Net Segmentation Head**.
        - *Data*: Trained on ground truth masks generated by **SAM3** (Segment Anything Model).
        - *Comparison*: We experimented with **Canny Edge Detection** (with Watershed) and Convex Hull. While effective in controlled lighting, they failed in challenging real-world lighting. MobileNetV2 will prove to be significantly more robust.
...
- **Gabor Filter, Skeletonization(Thinning Ridges), and Minutiae Detection**: it did not give good results,it is included in the application just for demonstration purposes.
---
...
### Excluded Advanced Features or Features with poor results
...
| **Neural Segmentation** | **Performance**: MobileNetV3 U-Net models need more training data for better performance. |
